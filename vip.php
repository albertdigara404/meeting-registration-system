...
include('./phpqrcode/qrlib.php');
    ...
    //create connection
     ...
     if(!$conn){
        ...
        $sql = "INSERT INTO vip_tb (name, id_number, Email, Country, Position) VALUES ('$name', '$passport', '$email', '$country', '$position')";
     }

     //if connection established and sql insert data function finished

     if (mysqli_query($conn, $sql)) {?>
        <script>alert("Meeting registration for VIP was a success. An email will be sent to you");
        </script>;
...
try {
    ...
    $mail->Subject = 'VIP Meeting Application';
    $mail->Body = '<p>Hello ' . $name . ',</p>
                   <p>Congratulations! Your KMA VIP meeting application was successful.</p>
    ...
    $mail->AltBody = 'The dates for the meeting will be sent to you in advance. Thank you';
     ...
     $pdf->Cell(0, 10, 'VIP Information:', 0, 1, 'C');
     ...
     $pdf->SetFont('helvetica', 'B', 14); 
     $pdf->Cell(0, 10, 'VIP Meeting Details:', 0, 1, 'C');
     $pdf->SetFont('helvetica', '', 12);
     $pdf->Ln(5);
     ...
     $pdfFilePath = (__DIR__ .'/pdfs/vip_information.pdf');
     $pdf->Output($pdfFilePath, 'F');
     $mail->addAttachment($pdfFilePath, 'VIP Information.pdf');
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h3> KMA Meeting Attendance VIP form </h3>
        </div>

        <form method = "post" action = "vip.php" id="contact-form">
            ...
            <input type="submit" class="input-btn bg-red" name="apply_vip">
