...
include('./phpqrcode/qrlib.php');
    ...
    //create connection
     ...
     if(!$conn){
        ...
        $sql = "INSERT INTO support_tb (name, id_number, Position, Email, Country ) VALUES ('$name', '$passport', '$position', '$email', '$country')";
     }
     //if connection established and sql insert data function finished

     if (mysqli_query($conn, $sql)) {?>
        <script>alert("Meeting registration for support was a success. An email will be sent to you");
        </script>;
...
try {
    ...
    $mail->Subject = 'Support Meeting Application';
    $mail->Body = '<p>Hello ' . $name . ',</p>
                   <p>Congratulations! Your KMA SUPPORT meeting application was successful.</p>
    ...
    $pdf->Cell(0, 10, 'Support Information:', 0, 1, 'C');
    ...
    $pdf->SetFont('helvetica', 'B', 14); 
    $pdf->Cell(0, 10, 'Support Meeting Details:', 0, 1, 'C');
    $pdf->SetFont('helvetica', '', 12);
    $pdf->Ln(5);
    ...
    $pdfFilePath = (__DIR__ .'/pdfs/support_information.pdf');
    $pdf->Output($pdfFilePath, 'F');
    $mail->addAttachment($pdfFilePath, 'Support Information.pdf');
...
<body>
    ...
    <div class="container">
        <div class="header">
            <h3> KMA Meeting Attendance Support form </h3>
        </div>

        <form method = "post" action = "support.php" id="contact-form">
            <label> Name </label>
            ...
            <input type="submit" class="input-btn bg-yellow" name="apply_support">
